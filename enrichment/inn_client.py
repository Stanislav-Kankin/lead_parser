...
from utils.domain_normalizer import domains_for_lookup, normalize_domain
    ...
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
        for candidate in candidates:
            ...
            if data:
                ...
                data.setdefault("lookup_strategy", "root_domain" if candidate != normalize_domain(domain) else "full_domain")
                return data
    return None
...
async def _lookup_domain(client: httpx.AsyncClient, domain: str) -> dict[str, Any] | None:
    ...
    if not normalized.get("inn") and not normalized.get("legal_name") and not normalized.get("email") and not normalized.get("phone"):
        return None
    return normalized
...
def _normalize_payload(data: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "okved": _clean_text(data.get("okved")),
    }
