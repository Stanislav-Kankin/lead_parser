...
def _build_outreach_text(signal) -> str:
    ...
    if any(token in haystack for token in ["возврат", "пвз", "не выкуп", "невыкуп", "срок хранения"]):
        hook = "увидел ваше сообщение про возвраты на ПВЗ."
        bridge = (
        ...
        value = (
            ...
            "и не вырос ли срок доставки. Из-за длинной доставки невыкупы обычно растут первыми."
        )
    elif any(token in haystack for token in ["упд", "усн", "расход", "отчет реализации", "фин отчет", "документ"]):
        hook = "увидел ваше сообщение про отчёты/документы WB."
        bridge = (
        ...
        value = (
            ...
            "Так быстрее видно, где именно расход расходится."
        )
        ...
        value = (
            ...
            "и не проваливается ли конверсия на фото/первом экране."
        )
        ...
        value = (
            ...
            "Обычно после этого видно, это проблема товара, категории или условий площадки."
        )
        ...
        value = (
            "Сначала обычно проверяют, есть ли повторный спрос и понятная экономика на уровне SKU. "
            "Если она есть, уже можно думать про сайт/Кит/Директ как тест, а не как большой переезд."
        )
    else:
    ...
    return (
        ...
        f"{bridge}\n\n"
        f"{value}\n\n"
        "Если интересно, могу чуть подробнее набросать варианты по вашей ситуации. Без продажи, просто по делу."
    )
